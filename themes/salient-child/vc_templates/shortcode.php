...
if( !function_exists('nectar_portfolio_processing') ) {
	...
	<?php
	$gallery_images = get_post_meta($post->ID, '_nectar_portfolio_gallery', true);
	if (!empty($gallery_images)) {
		...
		foreach ($gallery_images as $gallery_image_id) {
			...
			if (!empty($gallery_image_src)) {
				echo '<a href="' . esc_url($gallery_image_src[0]) . '" class="pretty_photo" rel="prettyPhoto[' . $post->ID . ']"></a>';
			}
		}
	} else {
